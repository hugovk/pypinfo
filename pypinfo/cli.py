...
@click.option('--load-json', '-lj', help='Show data from this JSON file.')
...
def pypinfo(
    ...
    if load_json:
        query_info, rows = load_json_from_file(load_json)
        bytes_billed = query_info['bytes_billed']
        bytes_processed = query_info['bytes_processed']
        estimated_cost = query_info['estimated_cost']
        from_cache = query_info['cached']
    else:
